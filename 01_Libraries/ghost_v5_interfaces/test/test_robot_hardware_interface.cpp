...
TEST_F(RobotHardwareInterfaceTestFixture, testConstructionNoThrow) {
  EXPECT_NO_THROW(
    RobotHardwareInterface hw_interface(
      device_config_map_ptr_single_joy_,
      hardware_type_e::COPROCESSOR));
  EXPECT_NO_THROW(
    RobotHardwareInterface hw_interface(
      device_config_map_ptr_dual_joy_,
      hardware_type_e::COPROCESSOR));
}

TEST_F(RobotHardwareInterfaceTestFixture, testEqualityOperator) {
  RobotHardwareInterface hw_interface_1(device_config_map_ptr_dual_joy_,
    hardware_type_e::COPROCESSOR);
  RobotHardwareInterface hw_interface_2(device_config_map_ptr_dual_joy_,
    hardware_type_e::COPROCESSOR);
...
TEST_F(RobotHardwareInterfaceTestFixture, testThrowsOnNonExistentDevice) {
  ...
  EXPECT_THROW(
    auto data = hw_interface.getDeviceData<MotorDeviceData>(
      "non_existent_motor"), std::runtime_error);
  EXPECT_THROW(hw_interface.setDeviceData(motor_data_ptr), std::runtime_error);
  EXPECT_THROW(
    auto config = hw_interface.getDeviceConfig<MotorDeviceConfig>(
      "non_existent_motor"), std::runtime_error);
}
...
TEST_F(RobotHardwareInterfaceTestFixture, testSingleJoystickThrowsWhenRequestingDualJoystick) {
  ...
  EXPECT_THROW(
    hw_interface.getDeviceData<JoystickDeviceData>(
      PARTNER_JOYSTICK_NAME), std::runtime_error);
}
...
TEST_F(RobotHardwareInterfaceTestFixture, testMotorStateGetters) {
  ...
  EXPECT_EQ(
    hw_interface.getMotorVelocityRPM("left_drive_motor"),
    motor_data_ptr->curr_velocity_rpm);
}
...
TEST_F(RobotHardwareInterfaceTestFixture, testRotationSensorStateGetters) {
  ...
  EXPECT_EQ(
    hw_interface.getRotationSensorAngleDegrees("rotation_sensor_1"),
    sensor_data_ptr->angle);
  EXPECT_EQ(
    hw_interface.getRotationSensorPositionDegrees(
      "rotation_sensor_1"), sensor_data_ptr->position);
  EXPECT_EQ(
    hw_interface.getRotationSensorVelocityRPM(
      "rotation_sensor_1"), sensor_data_ptr->velocity);
}
...
TEST_F(RobotHardwareInterfaceTestFixture, testSetMotorCurrentLimitMilliAmps) {
  ...
  EXPECT_EQ(
    hw_interface.getDeviceData<MotorDeviceData>(
      "left_drive_motor")->current_limit, rand_current);
...
TEST_F(RobotHardwareInterfaceTestFixture, testThrowsOnMismatchedDevices) {
  ...
  EXPECT_THROW(
    hw_interface.setMotorPositionCommand(
      "rotation_sensor_1",
      getRandomFloat()), std::runtime_error);
  EXPECT_THROW(
    hw_interface.setMotorVelocityCommandRPM(
      "rotation_sensor_1",
      getRandomFloat()), std::runtime_error);
  EXPECT_THROW(
    hw_interface.setMotorVoltageCommandPercent(
      "rotation_sensor_1",
      getRandomFloat()), std::runtime_error);
  EXPECT_THROW(
    hw_interface.setMotorTorqueCommandPercent(
      "rotation_sensor_1",
      getRandomFloat()), std::runtime_error);
  EXPECT_THROW(
    hw_interface.setMotorCurrentLimitMilliAmps(
      "rotation_sensor_1",
      getRandomFloat()), std::runtime_error);
  EXPECT_THROW(hw_interface.getMotorPosition("rotation_sensor_1"), std::runtime_error);
  EXPECT_THROW(
    hw_interface.setMotorCommand(
      "rotation_sensor_1", 0.0, 0.0, 0.0,
      0.0), std::runtime_error);
  EXPECT_THROW(
    hw_interface.setMotorControlMode(
      "rotation_sensor_1", false, false, false,
  ...
  EXPECT_THROW(
    hw_interface.getRotationSensorPositionDegrees("left_drive_motor"),
    std::runtime_error);
...
TEST_F(RobotHardwareInterfaceTestFixture, testThrowsOnBadDeviceCast) {
  ...
  EXPECT_THROW(
    hw_interface.getDeviceConfig<JoystickDeviceConfig>(
      "left_drive_motor"), std::runtime_error);
  EXPECT_THROW(
    hw_interface.getDeviceConfig<MotorDeviceConfig>(
      "rotation_sensor_1"), std::runtime_error);
  EXPECT_THROW(
    hw_interface.getDeviceConfig<RotationSensorDeviceConfig>(
      MAIN_JOYSTICK_NAME), std::runtime_error);

  EXPECT_THROW(
    hw_interface.getDeviceData<JoystickDeviceData>(
      "left_drive_motor"), std::runtime_error);
  EXPECT_THROW(
    hw_interface.getDeviceData<MotorDeviceData>("rotation_sensor_1"),
    std::runtime_error);
  EXPECT_THROW(
    hw_interface.getDeviceData<RotationSensorDeviceData>(
      MAIN_JOYSTICK_NAME), std::runtime_error);
}
