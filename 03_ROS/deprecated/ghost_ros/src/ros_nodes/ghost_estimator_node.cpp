...
GhostEstimatorNode::GhostEstimatorNode()
: Node("ghost_estimator_node")
{
...
void GhostEstimatorNode::LoadROSParams()
{
  ...
  left_wheel_link_ = Eigen::Vector2f(
    get_parameter(
      "odometry.left_wheel_x").as_double(), get_parameter("odometry.left_wheel_y").as_double());
  right_wheel_link_ = Eigen::Vector2f(
    get_parameter(
      "odometry.right_wheel_x").as_double(), get_parameter("odometry.right_wheel_y").as_double());
  back_wheel_link_ = Eigen::Vector2f(
    get_parameter(
      "odometry.back_wheel_x").as_double(), get_parameter("odometry.back_wheel_y").as_double());
...
void GhostEstimatorNode::CalculateOdometry(const ghost_msgs::msg::V5SensorUpdate::SharedPtr msg)
{
  ...
  float left_wheel_speed = (diff_swerve_jacobian * Eigen::Vector2f(
      ...
      msg->encoders[ghost_v5_config::DRIVE_LEFT_BACK_MOTOR].velocity_rpm))[0] * 2.75 * 3.14159 *
    2.54 / 100.0 / 60.0;
  float right_wheel_speed = (diff_swerve_jacobian * Eigen::Vector2f(
      ...
      msg->encoders[ghost_v5_config::DRIVE_RIGHT_BACK_MOTOR].velocity_rpm))[0] * 2.75 * 3.14159 *
    2.54 / 100.0 / 60.0;
  float rear_wheel_speed = (diff_swerve_jacobian * Eigen::Vector2f(
      ...
      msg->encoders[ghost_v5_config::DRIVE_BACK_LEFT_REAR_MOTOR].velocity_rpm))[0] * 2.75 *
    3.14159 * 2.54 / 100.0 / 60.0;

  Eigen::VectorXf vel_vectors(6);
  vel_vectors <<
    cos(
    ghost_common::WrapAngle360(
      msg->encoders[ghost_v5_config::STEERING_LEFT_ENCODER].
      position_degrees) * M_PI / 180.0) * left_wheel_speed,
    sin(
    ghost_common::WrapAngle360(
      msg->encoders[ghost_v5_config::STEERING_LEFT_ENCODER].
      position_degrees) * M_PI / 180.0) * left_wheel_speed,
    cos(
    ghost_common::WrapAngle360(
      msg->encoders[ghost_v5_config::STEERING_RIGHT_ENCODER].
      position_degrees) * M_PI / 180.0) * right_wheel_speed,
    sin(
    ghost_common::WrapAngle360(
      msg->encoders[ghost_v5_config::STEERING_RIGHT_ENCODER].
      position_degrees) * M_PI / 180.0) * right_wheel_speed,
    cos(
    ghost_common::WrapAngle360(
      msg->encoders[ghost_v5_config::STEERING_BACK_ENCODER].
      position_degrees) * M_PI / 180.0) * rear_wheel_speed,
    sin(
    ghost_common::WrapAngle360(
      msg->encoders[ghost_v5_config::STEERING_BACK_ENCODER].
      position_degrees) * M_PI / 180.0) * rear_wheel_speed;
...
void GhostEstimatorNode::PublishGhostRobotState(
  const ghost_msgs::msg::V5SensorUpdate::SharedPtr sensor_update_msg)
{
  ...
  ///// Steering States /////
  robot_state_msg.left_steering_angle =
    ghost_common::WrapAngle360(
    sensor_update_msg->encoders[ghost_v5_config::STEERING_LEFT_ENCODER].position_degrees);
  robot_state_msg.right_steering_angle =
    ghost_common::WrapAngle360(
    sensor_update_msg->encoders[ghost_v5_config::STEERING_RIGHT_ENCODER].position_degrees);
  robot_state_msg.back_steering_angle =
    ghost_common::WrapAngle360(
    sensor_update_msg->encoders[ghost_v5_config::STEERING_BACK_ENCODER].position_degrees);

  robot_state_msg.left_steering_vel =
    ghost_common::WrapAngle360(
    sensor_update_msg->encoders[ghost_v5_config::STEERING_LEFT_ENCODER].velocity_rpm);
  robot_state_msg.right_steering_vel =
    ghost_common::WrapAngle360(
    sensor_update_msg->encoders[ghost_v5_config::STEERING_RIGHT_ENCODER].velocity_rpm);
  robot_state_msg.back_steering_vel =
    ghost_common::WrapAngle360(
    sensor_update_msg->encoders[ghost_v5_config::STEERING_BACK_ENCODER].velocity_rpm);
...
void GhostEstimatorNode::CalculateHSpaceICR(ghost_msgs::msg::V5SensorUpdate::SharedPtr encoder_msg)
{
  ...
  if ((h_space_icr_points[0] - h_space_icr_points[1]).norm() >
    (h_space_icr_points[0] + h_space_icr_points[1]).norm())
  {
    h_space_icr_points[1] *= -1;
  }

  if ((h_space_icr_points[0] - h_space_icr_points[2]).norm() >
    (h_space_icr_points[0] + h_space_icr_points[2]).norm())
  {
  ...
  if (fabs(h_space_icr_avg_[2]) < 1e-9) {
    icr_flat_estimation_ = Eigen::Vector3f(
      std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity(), 0);
  } else {
    icr_flat_estimation_ = Eigen::Vector3f(
      h_space_icr_avg_[0] / h_space_icr_avg_[2],
      h_space_icr_avg_[1] / h_space_icr_avg_[2], 0);
  }

  // Initialize Visualization msg, and add debug visualization
  std::vector<geometry::Line2f> lines{left_encoder_vector, right_encoder_vector,
    back_encoder_vector};
...
void GhostEstimatorNode::DrawPredictedScan(visualization_msgs::msg::MarkerArray & viz_msg)
{
  ...
  for (std::size_t i = 0; i < predicted_scan.size(); i++) {
    ...
    if (!config_params.use_skip_range || (laser_index < config_params.skip_index_min) ||
      (laser_index > config_params.skip_index_max) )
    {
  ...
  for (std::size_t i = 0; i < last_laser_msg_->ranges.size(); i++) {
      ...
      (laser_index > config_params.skip_index_max) )
    {
      ...
      if ((range >= config_params.range_min) && (range <= config_params.range_max) ) {
        float angle = last_laser_msg_->angle_min + i * last_laser_msg_->angle_increment +
          config_params.laser_angle_offset + robot_angle;

        Eigen::Vector2f p =
          Eigen::Vector2f(
          range * cos(angle),
          range * sin(angle)) + robot_loc + rot_bl_to_world * Eigen::Vector2f(
          config_params.laser_offset, 0.0);
        auto point_msg = geometry_msgs::msg::Point{};
        point_msg.x = p.x();
        point_msg.y = p.y();
...
void GhostEstimatorNode::PublishJointStateMsg(const ghost_msgs::msg::V5SensorUpdate::SharedPtr msg)
{
  ...
  joint_state_msg.position.push_back(
    ghost_common::WrapAngle360(
      left_encoder.position_degrees) * M_PI / 180.0);
  joint_state_msg.position.push_back(0.0);
  joint_state_msg.position.push_back(
    ghost_common::WrapAngle360(
      right_encoder.position_degrees) * M_PI / 180.0);
  joint_state_msg.position.push_back(0.0);
  joint_state_msg.position.push_back(
    ghost_common::WrapAngle360(
      back_encoder.position_degrees) * M_PI / 180.0);
  joint_state_msg.position.push_back(0.0);
...
void GhostEstimatorNode::DrawICRPoints(std::vector<Eigen::Vector3f> & points)
{
  ...
  marker_msg_points.id = 4;
  marker_msg_points.action = 0;
  marker_msg_points.type = 7;
  marker_msg_points.scale.x = 0.01;
  marker_msg_points.scale.y = 0.01;
  marker_msg_points.scale.z = 0.01;
  marker_msg_points.color.b = 1;
  marker_msg_points.color.a = 1;
  ...
  marker_msg_sphere.id = 5;
  marker_msg_sphere.action = 0;
  marker_msg_sphere.type = 2;
  marker_msg_sphere.scale.x = 1;
  marker_msg_sphere.scale.y = 1;
  marker_msg_sphere.scale.z = 1;
  marker_msg_sphere.color.r = 0.75;
  marker_msg_sphere.color.g = 0.75;
  marker_msg_sphere.color.b = 0.75;
