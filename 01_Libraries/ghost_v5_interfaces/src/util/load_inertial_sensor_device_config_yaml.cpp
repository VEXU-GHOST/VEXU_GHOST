...
InertialSensorDeviceData::SerialConfig loadInertialSensorSerialConfigFromYAML(YAML::Node node, bool verbose){
...
void loadInertialSensorDeviceConfigFromYAML(YAML::Node node,
	...
	if(!device_node){
		throw std::runtime_error("[loadInertialSensorDeviceConfigFromYAML] Error: Inertial Sensor " + sensor_name + " not found!");
	}

	// Get name of sensor-specific config
	if(!device_node["config"]){
		throw std::runtime_error("[loadInertialSensorDeviceConfigFromYAML] Error: Inertial Sensor Config not found for sensor " + sensor_name + "!");
	}
	...
	if(!config_node){
		throw std::runtime_error("[loadInertialSensorDeviceConfigFromYAML] Error: Config not found for name " + config_name + "!");
	}

	if(config_node["serial"]){
		sensor_device_config_ptr->serial_config = loadInertialSensorSerialConfigFromYAML(config_node["serial"], verbose);
	}
	...
	if(device_node["type"].as<std::string>() != "INERTIAL_SENSOR"){
		throw std::runtime_error("[loadInertialSensorDeviceConfigFromYAML] Error: Device type is incorrect for Inertial Sensor " + sensor_name + "!");
	}

	if(!loadYAMLParam(device_node, "port", sensor_device_config_ptr->port, verbose)){
		throw std::runtime_error("[loadInertialSensorDeviceConfigFromYAML] Error: Port not specified for Inertial Sensor " + sensor_name + "!");
	}
}
