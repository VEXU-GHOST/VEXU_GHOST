...
public:
  struct SerialConfig
  {
    ...
    {
      return (send_position_command == rhs.send_position_command) &&
             (send_velocity_command == rhs.send_velocity_command) &&
             (send_voltage_command == rhs.send_voltage_command) &&
             (send_torque_command == rhs.send_torque_command) &&
             (send_torque_data == rhs.send_torque_data) &&
             (send_voltage_data == rhs.send_voltage_data) &&
             (send_current_data == rhs.send_current_data) &&
             (send_power_data == rhs.send_power_data) &&
  ...
  bool operator==(const DeviceBase & rhs) const override
  {
    ...
    return (d_rhs != nullptr) && (name == d_rhs->name) && (type == d_rhs->type) &&
           (position_command == d_rhs->position_command) &&
           (velocity_command == d_rhs->velocity_command) &&
           (torque_command == d_rhs->torque_command) &&
           (voltage_command == d_rhs->voltage_command) &&
           (current_limit == d_rhs->current_limit) &&
           (position_control == d_rhs->position_control) &&
           (velocity_control == d_rhs->velocity_control) &&
           (torque_control == d_rhs->torque_control) &&
           (voltage_control == d_rhs->voltage_control) && (curr_position == d_rhs->curr_position) &&
           (curr_velocity_rpm == d_rhs->curr_velocity_rpm) &&
           (curr_torque_nm == d_rhs->curr_torque_nm) &&
           (curr_voltage_mv == d_rhs->curr_voltage_mv) &&
           (curr_current_ma == d_rhs->curr_current_ma) &&
  ...
  std::vector<unsigned char> serialize(hardware_type_e hardware_type) const override
  {
    ...
    if (hardware_type == hardware_type_e::COPROCESSOR) {
      ...
      byte_offset++;
      checkMsgSize(byte_offset, getActuatorPacketSize());
      ...
      throw std::runtime_error(
              "[MotorDeviceData::deserialize] Error: Received unsupported hardware type " + std::to_string(
                hardware_type) + " on motor " + name);
    }
    return msg;
  }
...
public:
  ...
  bool operator==(const DeviceBase & rhs) const override
  {
    ...
    return (d_rhs != nullptr) && (port == d_rhs->port) && (name == d_rhs->name) &&
           (type == d_rhs->type) &&
           (reversed == d_rhs->reversed) && (gearset == d_rhs->gearset) &&
           (brake_mode == d_rhs->brake_mode) &&
           (encoder_units == d_rhs->encoder_units) &&
           (controller_config == d_rhs->controller_config) &&
