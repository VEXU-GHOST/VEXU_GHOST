...
void SwerveModel::calculateJacobians()
{
    ...
    case swerve_type_e::DIFFERENTIAL:
      {
        ...
        m_module_jacobian_inv << 1 / m_config.wheel_ratio, 1 / m_config.steering_ratio,
          -1 / m_config.wheel_ratio, 1 / m_config.steering_ratio;
      }
      break;
  }
  ...
  m_task_space_jacobian =
    m_task_space_jacobian_inverse.completeOrthogonalDecomposition().pseudoInverse();
  ...
  double clockwise_cmd)
{
  ...
  double lin_vel_cmd = std::clamp<double>(
    xy_vel_cmd_base_link.norm(), -m_max_base_lin_vel, m_max_base_lin_vel);
  ...
  for (const auto & [module_name, module_position] : m_config.module_positions) {
    ...
    module_command.steering_angle_command =
      ghost_util::WrapAngle360(
      atan2(
        velocity_vector.y(),
        velocity_vector.x()) * ghost_util::RAD_TO_DEG);
    ...
    double steering_error = ghost_util::SmallestAngleDistDeg(
      module_command.steering_angle_command,
      steering_angle);
    ...
    if (fabs(steering_error) > 90.0) {
      ...
      module_command.steering_angle_command = ghost_util::FlipAngle180(
        module_command.steering_angle_command);

      // Recalculate error
      steering_error = ghost_util::SmallestAngleDistDeg(
        module_command.steering_angle_command,
        steering_angle);
    }
    ...
    m_error_sum_map[module_name] = ghost_util::clamp<double>(
      m_error_sum_map[module_name],
      -m_config.steering_ki_limit,
      m_config.steering_ki_limit);                                                                                                                                       // CHANGED
  ...
  for (auto & [name, command] : m_module_commands) {
    ...
    auto module_vel_cmd = Eigen::Vector2d(
      command.wheel_velocity_command,
      command.steering_velocity_command);
    command.actuator_velocity_commands = m_module_jacobian_inv * module_vel_cmd;

    auto module_voltage_cmd = Eigen::Vector2d(
      command.wheel_voltage_command,
      command.steering_voltage_command);
