...
public:
  ...
  : Node("Sensor_node")
  {
    publisher_ = this->create_publisher<std_msgs::msg::String>("Data", 10);     //stores 10 readings for averaging idk
    ...
    this->declare_parameter<int>("address", 0x44);            // example
    this->declare_parameter<std::string>("i2c_device", "/dev/i2c-7");
    ...
    color_sensor = std::make_shared<ghost_sensing::ISL29125>(iface, address);
...
private:
  void timer_callback()
  {
    ...
    if (!color_sensor->readAllSensors(data)) {
   ...
   uint8_t& ctrl = val;
   //color_sensor->Init();
   color_sensor->readRegister(0x01, ctrl);
    //message.data = std::to_string(val);
...
int main(int argc, char * argv[])
{
  ...
  //RCLCPP_INFO(this->get_logger(), "Initialized ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd");
