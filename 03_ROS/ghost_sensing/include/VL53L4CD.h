...
class VL53L4CD {
    ...
    writeRegisters(uint8_t reg, uint8_t *buf, uint16_t len);
    readRegister(uint8_t reg, uint8_t &data);   
    readRegisters(uint8_t reg, uint8_t *buf, uint16_t len);

    public:
    VL53L4CD(std::shared_ptr<I2C_interfacing> iface, uint8_t address)
    ~VL53L4CD() = default;

    uint8_t init(dev_t dev);
    bool deinit();
   ...
   uint8_t VL53L4CD_CalibrateOffset(Dev_t dev, int16_t TargetDistInMm, int16_t *p_measured_offset_mm,
            int16_t nb_samples);
