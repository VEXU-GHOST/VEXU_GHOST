...
TEST_F(SwerveModelTestFixture, testICRZeroCaseRandomDirections) {
  m_diff_model_ptr->setModuleState(
    "front_right",
    ModuleState(0.0, 45.0 + 180.0 * getRandomInt(10), 0.0, 0.0));
  m_diff_model_ptr->setModuleState(
    "front_left",
    ModuleState(0.0, -45.0 + 180.0 * getRandomInt(10), 0.0, 0.0));
  m_diff_model_ptr->setModuleState(
...
TEST_F(SwerveModelTestFixture, testICRAtBackLeft) {
  ...
  EXPECT_FALSE(m_diff_model_ptr->getICR(icr_point));
  EXPECT_TRUE(
    (icr_point - m_diff_model_ptr->getConfig().module_positions.at(
      "back_left")).norm() < 1e-10);
  EXPECT_TRUE(m_diff_model_ptr->getICRSSE() < 1e-10);
}
...
TEST_F(SwerveModelTestFixture, testICRInfinityNoisy) {
  m_diff_model_ptr->setModuleState(
    "front_right",
    ModuleState(0.0, 90.0 + getRandomDouble(0.05), 0.0, 0.0));
  m_diff_model_ptr->setModuleState(
    "front_left",
    ModuleState(0.0, 90.0 + getRandomDouble(0.05), 0.0, 0.0));
  m_diff_model_ptr->setModuleState(
...
TEST_F(SwerveModelTestFixture, generatePlots) {
  ...
  if (GENERATE_PLOTS) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      m_diff_model_ptr->setModuleState(
        "front_right",
        ModuleState(0.0, 90.0 + getRandomDouble(noise_max), 0.0, 0.0));
      m_diff_model_ptr->setModuleState(
        "front_left",
        ModuleState(0.0, 90.0 + getRandomDouble(noise_max), 0.0, 0.0));
      m_diff_model_ptr->setModuleState(
