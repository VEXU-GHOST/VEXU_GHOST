  ...
  last_ang_vel_cmd_{0.0}
{
  ...
  left_wheel_pos_ =
    Eigen::Vector2f(
    get_parameter("left_wheel_x").as_double(), get_parameter(
      "left_wheel_y").as_double());
  right_wheel_pos_ =
    Eigen::Vector2f(
    get_parameter("right_wheel_x").as_double(), get_parameter(
      "right_wheel_y").as_double());
  back_wheel_pos_ =
    Eigen::Vector2f(
    get_parameter("back_wheel_x").as_double(), get_parameter(
      "back_wheel_y").as_double());
...
void SpinUpRobotStateMachineNode::teleop()
{
  ...
  float des_angle = M_PI / 180.0 *
    ghost_common::WrapAngle360(
    atan2(
      y_goal_ - curr_robot_state_msg_->y,
      x_goal_ - curr_robot_state_msg_->x) * 180.0 / M_PI);
  float des_x = 0.0;
  float des_y = 0.0;

  if (curr_joystick_msg_->joystick_btn_l1) {
    float ang_vel_cmd =
      ghost_common::SmallestAngleDistDeg(
      des_angle * 180 / M_PI,
      curr_robot_state_msg_->theta * 180 / M_PI) * M_PI / 180.0 *
      rotate_kp_ -
    ...
    if (sqrt(
        ...
        fabs(des_y - curr_robot_state_msg_->y) * fabs(des_y - curr_robot_state_msg_->y)) <
    ...
    auto rotate_base_to_world =
      Eigen::Rotation2D<float>(-curr_robot_state_msg_->theta).toRotationMatrix();
  ...
  switch (teleop_mode) {
    case SHOOTER_MODE:
      ...
      actuator_cmd_msg_.motor_commands[ghost_v5_config::SHOOTER_RIGHT_MOTOR].desired_velocity =
        1200;
  ...
  float y_velocity)
{
  ...
  std::vector<float> steering_angles = {
    ghost_common::WrapAngle360(
      curr_encoder_msg_->encoders[ghost_v5_config::STEERING_LEFT_ENCODER].position_degrees),
    ghost_common::WrapAngle360(
      curr_encoder_msg_->encoders[ghost_v5_config::STEERING_RIGHT_ENCODER].position_degrees),
    ghost_common::WrapAngle360(
      curr_encoder_msg_->encoders[ghost_v5_config::STEERING_BACK_ENCODER].position_degrees),
  };
  ...
  std::vector<Eigen::Vector2f> wheel_positions =
  {left_wheel_pos_, right_wheel_pos_, back_wheel_pos_};
  for (int wheel_id = 0; wheel_id < 3; wheel_id++) {
    ...
    steering_angle_cmd_[wheel_id] = ghost_common::WrapAngle360(
      atan2(
        vel_vec.y(),
        vel_vec.x()) * 180.0 / M_PI);                                                                                         // Converts rad/s to degrees
    ...
    float steering_error = ghost_common::SmallestAngleDistDeg(
      steering_angle_cmd_[wheel_id],
      steering_angles[wheel_id]);
    ...
    if (fabs(steering_error) > 90.0) {
      ...
      // Recalculate error
      steering_error = ghost_common::SmallestAngleDistDeg(
        steering_angle_cmd_[wheel_id],
        steering_angles[wheel_id]);
    }
  ...
  publishSwerveKinematicsVisualization(
    wheel_vel_vectors[0], wheel_vel_vectors[1],
    wheel_vel_vectors[2]);

  // Calculate Actuator Commands
  Eigen::Matrix2f diff_swerve_jacobian;
  Eigen::Matrix2f diff_swerve_jacobian_inverse;
  diff_swerve_jacobian << 13.0 / 18.0 / 2.0, -13.0 / 18.0 / 2.0, 13.0 / 45.0 / 2.0,
    13.0 / 45.0 / 2.0;
  ...
  for (int wheel_id = 0; wheel_id < 3; wheel_id++) {
    ...
    Eigen::Vector2f wheel_module_vel_cmd(wheel_velocity_cmd_[wheel_id],
      steering_velocity_cmd_[wheel_id]);
    ...
    Eigen::Vector2f wheel_module_voltage_cmd(wheel_voltage_cmd_[wheel_id],
      steering_voltage_cmd_[wheel_id]);
    Eigen::Vector2f actuator_voltage_cmd = diff_swerve_jacobian.transpose() *
      wheel_module_voltage_cmd;
  ...
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_LEFT_FRONT_MOTOR].desired_velocity =
    motor_speed_cmds[0];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_LEFT_BACK_MOTOR].desired_velocity =
    motor_speed_cmds[1];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_RIGHT_FRONT_MOTOR].desired_velocity =
    motor_speed_cmds[2];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_RIGHT_BACK_MOTOR].desired_velocity =
    motor_speed_cmds[3];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_RIGHT_REAR_MOTOR].desired_velocity =
    motor_speed_cmds[4];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_RIGHT_FRONT_MOTOR].desired_velocity =
    motor_speed_cmds[4];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_LEFT_REAR_MOTOR].desired_velocity =
    motor_speed_cmds[5];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_LEFT_FRONT_MOTOR].desired_velocity =
    motor_speed_cmds[5];

  // Update voltage commands
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_LEFT_FRONT_MOTOR].desired_voltage =
    motor_voltage_cmds[0];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_LEFT_BACK_MOTOR].desired_voltage =
    motor_voltage_cmds[1];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_RIGHT_FRONT_MOTOR].desired_voltage =
    motor_voltage_cmds[2];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_RIGHT_BACK_MOTOR].desired_voltage =
    motor_voltage_cmds[3];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_RIGHT_REAR_MOTOR].desired_voltage =
    motor_voltage_cmds[4];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_RIGHT_FRONT_MOTOR].desired_voltage =
    motor_voltage_cmds[4];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_LEFT_REAR_MOTOR].desired_voltage =
    motor_voltage_cmds[5];
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_LEFT_FRONT_MOTOR].desired_voltage =
    motor_voltage_cmds[5];
  ...
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_RIGHT_REAR_MOTOR].current_limit =
    2500;
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_RIGHT_FRONT_MOTOR].current_limit =
    2500;
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_LEFT_REAR_MOTOR].current_limit =
    2500;
  actuator_cmd_msg_.motor_commands[ghost_v5_config::DRIVE_BACK_LEFT_FRONT_MOTOR].current_limit =
    2500;
