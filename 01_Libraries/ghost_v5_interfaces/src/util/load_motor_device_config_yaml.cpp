  ...
  bool verbose)
{
  ...
  if (!device_node) {
    throw std::runtime_error(
            "[loadMotorDeviceConfigFromYAML] Error: Motor " + motor_name + " not found!");
  }

  // Get name of motor-specific config
  if (!device_node["config"]) {
    throw std::runtime_error(
            "[loadMotorDeviceConfigFromYAML] Error: Motor Config not found for motor " + motor_name +
            "!");
  }
  ...
  if (!config_node) {
    throw std::runtime_error(
            "[loadMotorDeviceConfigFromYAML] Error: Motor Config not found for name " + config_name +
            "!");
  }
  ...
  if (device_node["type"].as<std::string>() != "MOTOR") {
    throw std::runtime_error(
            "[loadMotorDeviceConfigFromYAML] Error: Device type is incorrect for motor " + motor_name +
            "!");
  }

  if (!loadYAMLParam(device_node, "port", motor_device_config_ptr->port, false)) {
    throw std::runtime_error(
            "[loadMotorDeviceConfigFromYAML] Error: Port not specified for motor " + motor_name +
            "!");
  }
  ...
  if (config_node["controller"]) {
    motor_device_config_ptr->controller_config =
      loadMotorControllerConfigFromYAML(config_node["controller"]);
  }
