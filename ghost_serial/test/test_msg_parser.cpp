...
TEST_F(TestMsgParser, testMsgBasic){
    ...
    // Msg Parser
    auto msg_parser = ghost_serial::MsgParser(msg_len, start_seq);

    // Apply COBS Encoding to incoming Msg
    unsigned char encoded_input_buffer[cobs_packet_length] = {0,};
    COBS::cobsEncode(input_buffer, start_seq.length() + msg_len, encoded_input_buffer);
...
TEST_F(TestMsgParser, testMsgLengthExceeded){
    ...
    ASSERT_TRUE(msg_parser.parseByteStream(
        input_buffer2,
        sizeof(input_buffer2)/sizeof(input_buffer2[0]),
        output_buffer));
