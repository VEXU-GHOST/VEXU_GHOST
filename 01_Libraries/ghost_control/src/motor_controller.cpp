...
float MotorController::updateMotor(float position, float velocity){
	...
	float position_feedback = (position_active_) ? (des_pos_encoder_ - position) * controller_config_.pos_gain : 0.0;
	float velocity_feedforward = (velocity_active_) ? motor_model_.getVoltageFromVelocityMillivolts(des_vel_rpm_) * controller_config_.ff_vel_gain : 0.0;
	float velocity_feedback = (velocity_active_) ? (des_vel_rpm_ - curr_vel_rpm) * controller_config_.vel_gain : 0.0;
	float torque_feedforward = (torque_active_) ? motor_model_.getVoltageFromTorqueMillivolts(des_torque_nm_) * controller_config_.ff_torque_gain : 0.0;
