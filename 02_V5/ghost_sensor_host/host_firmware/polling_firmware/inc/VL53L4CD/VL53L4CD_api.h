...
/**
 * @brief Convenience function that populates a device handle, verifies the
 *        sensor ID (0xEBAA), and runs VL53L4CD_SensorInit().  The I2C bus
 *        must already be initialised before calling this function.
 * @param (Dev_t) dev : Device instance to populate and initialise.
 * @param (i2c_inst_t*) i2c : Pico SDK I2C peripheral (i2c0 or i2c1).
 * @param (uint8_t) address : 7-bit I2C address of the sensor (default 0x29).
 * @return (VL53L4CD_Error) status : 0 if OK.
 */

VL53L4CD_Error vl53l4cd_init(
		Dev_t dev,
		i2c_inst_t *i2c,
		uint8_t address);
