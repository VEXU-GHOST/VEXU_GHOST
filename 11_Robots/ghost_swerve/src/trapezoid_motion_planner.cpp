...
void TrapezoidMotionPlanner::computeTrapezoidFunction(double accel, double v_max, std::vector<double> vec_q0, std::vector<double> vec_qf){
	...
	if(time_2 < time_1){// doesnt reach max velocity
		if(abs(time_accel_final) <= 0.001){// divide by zero
			...
			computeTriangleFunction(pos_initial, pos_final, vel_initial, vel_final, accel_initial, accel_final);
		}
	}
	else{
		...
		double pc3 = accel_final * time_2 * time_2 / 2.0 + (vel_initial - v_max) * time_1 / 2.0 + pos_initial;

		trapezoidVelocityFunc_ = [time_1, time_2, time_3, va1, vb1, va2, vb2, va3, vb3, vel_final](double time){
									 if(time <= time_1){
										 return va1 * time
										        + vb1;
									 }
		...
		trapezoidPositionFunc_ = [time_1, time_2, time_3, pa1, pb1, pc1, pa2, pb2, pc2, pa3, pb3, pc3, pos_final](double time){
									 if(time <= time_1){
...
void TrapezoidMotionPlanner::computeTriangleFunction(double pos_initial, double pos_final,
	...
	if(determinant < 0.0){
		...
		trapezoidPositionFunc_ = [time_1, time_2, pa1, pb1, pc1, pa2, pb2, pc2, pos_final](double time){
									 if(time <= time_1){
										 return pa1 * time * time
										        + pb1 * time
										        + pc1;
...
void TrapezoidMotionPlanner::computeLinearFunction(double pos_initial, double pos_final,
                                                   double vel_initial, double vel_final,
