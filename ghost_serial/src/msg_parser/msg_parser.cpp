...
{
    MsgParser::MsgParser(int max_msg_len, std::string msg_start_seq, bool use_checksum) : max_msg_len_(max_msg_len),
    ...
    bool MsgParser::parseByteStream(const unsigned char raw_data_buffer[], const int num_bytes, unsigned char parsed_msg[], int &parsed_msg_len)
    {
            ...
            if (start_seq_index_ == msg_start_seq_.length()) // Found start sequence, now reading msg
            {
                ...
                if (msg_packet_index_ < max_msg_len_ + use_checksum_ + 2)
                {
                    if (raw_data_buffer[i] == 0x00) // Msg end delimiter
                    {
                        ...
                        COBS::cobsDecode(incoming_msg_buffer_.data(), parsed_msg_len + use_checksum_ + 2, decoded_msg);
